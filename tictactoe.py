...
def main():
    ...
    while True:
        ...
        if isWinner(gameBoard, currentPlayer):  # 먼저 승리를 확인한다
            ...
            break
        elif isBoardFull(gameBoard):  # 다음으로 무승부인지 확이한다
            print(getBoardStr(gameBoard))
            print("무승부 게임입니다!")
            break
...
def isWinner(board, player):
    ...
    return (
        (b["1"] == b["2"] == b["3"] == p)  # 상단에 걸쳐서
        or (b["4"] == b["5"] == b["6"] == p)  # 중단에 걸쳐서
        or (b["7"] == b["8"] == b["9"] == p)  # 하단에 걸쳐서
        or (b["1"] == b["4"] == b["7"] == p)  # 좌측 열 아래로
        or (b["2"] == b["5"] == b["8"] == p)  # 중앙 열 아래로
        or (b["3"] == b["6"] == b["9"] == p)  # 우측 열 아래로
        or (b["3"] == b["5"] == b["7"] == p)  # 대각선
        or (b["1"] == b["5"] == b["9"] == p)  # 대각선
    )
