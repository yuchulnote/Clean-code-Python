"""사목 작성자: 유철
입체사목과 비슷한, 타일을 떨어뜨려서 4개를 한 줄로 늘어놓는 게임"""
...
두 사람이 차례로 7개 열 중에 하나에 타일을 떨어뜨려
수평, 수직, 대각선으로 사목을 만든다.
"""
    )
...
def getPlayerMove(playerTile, board):
    ...
    while True:  # 플레이어가 유효한 움직임 명령을 입력할 때까지 계속 요청한다
        print(f"플레이어 {playerTile}, 1에서 {BOARD_HEIGHT}까지 숫자 또는 QUIT를 입력하십시오:")
